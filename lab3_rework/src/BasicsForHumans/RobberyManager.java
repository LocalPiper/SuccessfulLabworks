...
public class RobberyManager {
    ...
    public void preparation(Building building) {
        ...
        System.out.println(victims.getName() + " were working at " + building.getName() + " when " + bandits.getName() + " arrived!");
    ...
    public void shooting(Team attackers, Team defenders) {
        ...
        for (Human a : attackers.getMembers()) {
            Human attacker = attackers.getMembers()[(int) (Math.random() * (attackers.getMembers().length - 1))];
            Human defender = defenders.getMembers()[(int) (Math.random() * (defenders.getMembers().length - 1))];
